...
before(async () => {
  ...
  const doc = fs.readFileSync(path.join(process.cwd(), "src/data/knowledge/mahsumah-cloud/deployment-guide.md"), "utf8");
  await ingestDocument({ productId: "mahsumah-cloud", type: "markdown", title: "دليل النشر", text: doc, reference: "deployment-guide.md" });
...
test("answers a documented question with sources and confidence", async () => {
  const r = await runAgent({ productId: "mahsumah-cloud", tenantId: "t1", userId: "u1", role: "developer", message: "كيف أنشر تطبيق Next.js؟" });
...
test("uses a tool when the role permits and recommends a plan", async () => {
  const r = await runAgent({ productId: "mahsumah-cloud", tenantId: "t1", userId: "u1", role: "customer_admin", message: "ما هي أفضل باقة لموقعي؟" });
...
test("refuses account tools for roles without permission", async () => {
  const r = await runAgent({ productId: "mahsumah-cloud", tenantId: "t1", userId: "u9", role: "visitor", message: "ما هي فاتورتي القادمة؟" });
  assert.equal(r.toolCalls.length, 0);
  assert.equal(r.handoffRequired, true);
});

test("does not leak knowledge across products", async () => {
  const r = await runAgent({ productId: "mahsumah-dcc", tenantId: "t2", userId: "u2", role: "customer", message: "كيف أنشر تطبيق Next.js؟" });
  assert.equal(r.sources.length, 0);
  assert.equal(r.handoffRequired, true);
});

test("conversation continuity is scoped to the same tenant/user", async () => {
  const a = await runAgent({ productId: "mahsumah-cloud", tenantId: "t1", userId: "u1", role: "customer", message: "أين توجد الخوادم؟" });
  const b = await runAgent({ productId: "mahsumah-cloud", tenantId: "OTHER", userId: "u1", role: "customer", message: "تابع", conversationId: a.conversationId });
  assert.notEqual(a.conversationId, b.conversationId);
});

test("rejects roles not allowed for the product", async () => {
  await assert.rejects(() => runAgent({ productId: "mahsumah-dcc", tenantId: "t2", userId: "u2", role: "developer", message: "hi" }), /not allowed/);
});
